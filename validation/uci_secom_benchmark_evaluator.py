#!/usr/bin/env python3
"""
AgniPariksha 2.4 — Live Public UCI SECOM Benchmark Evaluator
==========================================================
Evaluates AgniPariksha's Module A & Robust Median-MAD Resolver on the 
...
3. AgniPariksha 2.4 Module A Robust Median-MAD Outlier Screener
"""
...
# -------------------------------------------------------------
# 3. AgniPariksha 2.4 Module A Robust Median-MAD Screener
# -------------------------------------------------------------
median_vals = X_clean.median()
mad_vals = (X_clean - median_vals).abs().median() + 1e-5
z_robust = (X_clean - median_vals) / (1.4826 * mad_vals)

# AgniPariksha Multi-Channel Robust Screener (Threshold Z_robust >= 2.5)
agnipariksha_flags = (z_robust.abs() >= 2.5).any(axis=1).astype(int)

ag_tp = int(((agnipariksha_flags == 1) & (y_true_binary == 1)).sum())
ag_fn = int(((agnipariksha_flags == 0) & (y_true_binary == 1)).sum())
ag_fp = int(((agnipariksha_flags == 1) & (y_true_binary == 0)).sum())
ag_tn = int(((agnipariksha_flags == 0) & (y_true_binary == 0)).sum())
...
print(f"  • AgniPariksha 2.4 Screener: Recall = {ag_recall:.1f}% | Escape Rate = {ag_escape:.1f}%")
print("=" * 80)
