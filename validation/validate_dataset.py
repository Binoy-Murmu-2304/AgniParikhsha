"""
AgniPariksha 3.0 - MIL-STD-883 & ESCC 9000 Alignment & Audit Validator
...
from models.anomaly_detector import AgniParikshaMLEngine


def load_manifest():
    return pd.read_csv("D:/SIH 2026/agnipariksha_core/data/ASQD_manifest.csv")
...
def run_mil_std_883_audit() -> dict:
    ...
    for _, r in manifest.iterrows():
        chk_path = os.path.join("agnipariksha_core", "data", f"{r['lot_id']}.csv")
    ...
    if not all_lots:
        ...
        lot_files = [f for f in os.listdir("agnipariksha_core/data") if f.startswith("LOT_") and f.endswith(".csv")]
        for f in lot_files:
            all_lots.append(pd.read_csv(os.path.join("agnipariksha_core", "data", f)))
    ...
    audit_report = {
        "platform": "AgniPariksha 3.0 Hybrid Reliability Intelligence Platform",
