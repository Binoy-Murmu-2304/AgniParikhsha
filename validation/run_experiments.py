"""
AgniPariksha 2.1 — Experiment Runner & Blind Evaluation (Phase 4 + 5 + 6)
=========================================================================
...
from models.anomaly_detector import (
    ...
    AgniParikshaMLEngine, evaluate_binary,
    _robust_z,
)

DATA_DIR = Path("D:/SIH 2026/agnipariksha_core/data")
...
def experiment_1(engine: AgniParikshaMLEngine, blind_df: pd.DataFrame):
...
def experiment_3(engine: AgniParikshaMLEngine):
...
def experiment_4(blind_df: pd.DataFrame, engine: AgniParikshaMLEngine):
...
def experiment_7(blind_df: pd.DataFrame, engine: AgniParikshaMLEngine):
    """
...
def stress_test_a_lot_shift(engine: AgniParikshaMLEngine):
...
def stress_test_b_measurement_noise(engine: AgniParikshaMLEngine, blind_df: pd.DataFrame):
...
def stress_test_c_unknown_failure_mode(train_lots: list):
    ...
    engine_ood = AgniParikshaMLEngine()
...
def experiment_8_known_vs_unknown(train_lots: list):
    ...
    Protocol:
      TRAIN: AgniParikshaMLEngine trained on Classes 0–5 (all known archetypes)
    ...
    Research Question:
      "Can AgniPariksha's unsupervised temporal layer detect previously
    ...
    engine_exp8 = AgniParikshaMLEngine()
...
def main():
    ...
    print_header("AgniPariksha 3.0 - Hybrid Intelligence Validation Suite")
    ...
    print_header("Experiment Suite Complete — AgniPariksha 3.0")
