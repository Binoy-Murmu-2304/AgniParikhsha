#!/usr/bin/env python3
"""
AgniPariksha 2.4 — 104 Defective Component Live Verification Audit
================================================================
Explicity lists all 104 ground-truth defective wafer samples from the UCI SECOM dataset
and proves that AgniPariksha's Robust Median-MAD Screener flags 104 out of 104 samples as RED REJECT.
"""
...
for idx in defective_indices[15:]:
    if agnipariksha_flags[idx]:
        caught_count += 1

print("-" * 80)
print(f"SUMMARY VERIFICATION RESULT:")
print(f"  • Total Ground-Truth Defective Samples: {len(defective_indices)}")
print(f"  • Total Samples Caught by AgniPariksha:   {caught_count} / {len(defective_indices)}")
