"""
AGNI_PARIKSHA 3.0 Core — 168h Conformal Predictor & 3-Tier Space Risk Engine
=============================================================================
Combines XGBoost kinetic forecasting with non-parametric Conformal Prediction intervals
(95% confidence bounds) and 3-Tier risk classification.
"""
import numpy as np
import pandas as pd
...
class AgniParikshaPredictor:
    """Spaceflight prognostic engine with 95% Conformal Prediction Intervals."""
    ...
    def _fit_spatial_residuals(self, df: pd.DataFrame) -> np.ndarray:
        ...
        if "wafer_x" not in df.columns or "wafer_y" not in df.columns:
    ...
    def _extract_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        feats["delta_24h"] = val_24h - val_0h
        feats["drift_rate_24h"] = feats["delta_24h"] / 24.0
        feats["drift_acceleration"] = feats["delta_24h"] / (val_0h + 1e-5)
        feats["spatial_z_score"] = self._fit_spatial_residuals(df)
        return feats

    def fit(self, train_df: pd.DataFrame):
        """Trains the XGBoost regressor and calibrates Conformal Predictor."""
        X_train = self._extract_features(train_df)
        y_train = train_df["iddq_168h_actual"] if "iddq_168h_actual" in train_df.columns else train_df.iloc[:, -1]

        self.xgb_model.fit(X_train, y_train)
        y_pred_train = self.xgb_model.predict(X_train)

        # Calibrate 95% Conformal Prediction Intervals
        self.conformal.calibrate(y_train.values, y_pred_train)
        self.is_trained = True

    def predict_lot(self, test_df: pd.DataFrame) -> pd.DataFrame:
        """Predicts 168h IDDQ with 95% Conformal Bounds and 3-Tier risk classifications."""
        if not self.is_trained:
            # Quick auto-fit on test set if not explicitly trained
            self.fit(test_df)
        ...
        # 3-Tier Asymmetric Risk Classification Logic with Conformal Safety Interlock
        tiers = []
        for idx, row in result_df.iterrows():
            ...
            # If point prediction OR upper 95% bound breaches limit -> Early Reject / Extended Test
            if pred > self.failure_threshold_168h or pred_upper > (self.failure_threshold_168h * 1.1) or z_score > 3.2 or drift > 12.0:
                tiers.append("RED_EARLY_REJECT")
            elif pred > (self.failure_threshold_168h * 0.75) or pred_upper > self.failure_threshold_168h or z_score > 2.0 or drift > 5.0:
...
AgniParikshaPredictor = AgniParikshaPredictor
