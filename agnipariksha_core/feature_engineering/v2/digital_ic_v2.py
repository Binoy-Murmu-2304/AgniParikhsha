"""
AgniPariksha 2.4 — Feature Engineering v2: Digital IC (0h + 24h + 96h)
======================================================================
Extends v1 contract with five 96h trajectory features.
V1 feature list is a strict subset — existing positions are preserved.
New features appended at the end to avoid index drift.

96h features justified by separability experiment:
  - THERMAL_RUNAWAY   : vel_24_96 Cohen d = 274.5
  - ELECTROMIGRATION  : growth_ratio Cohen d = 41.4
  - SPATIAL_OUTLIER   : accel Cohen d = 20.9
"""

from typing import List, Dict, Any, Optional
import pandas as pd
import numpy as np
from agnipariksha_core.feature_engineering.base import BaseFeatureEngineer
from agnipariksha_core.feature_engineering.common import (
    calc_growth_rate,
    calc_ratio,
...
class DigitalICFeatureEngineerV2(BaseFeatureEngineer):
    """
    V2 feature contract for DIGITAL_IC — adds 96h trajectory features.
    V1 features: positions [0..13] unchanged.
    V2 additions: positions [14..18].
    """
    ...
    def feature_names(self) -> List[str]:
        return [
            # --- v1 features (unchanged positions) ---
            "value_0h",
            "value_24h",
            "delta_value_0_24h",
    ...
    def extract_features(
        ...
        v96 = df["value_96h"]  # Must be present for v2
        ...
        res = pd.DataFrame({
            ...
            "temp_normalized_0h": t_norm_0, "temp_normalized_24h": t_norm_24,
            "robust_z_score_0h": z0, "robust_z_score_24h": z24,
            "ICC_active_mA": icc, "input_leakage_nA": leakage,
            "propagation_delay_ns": delay, "v_offset_24h": v_off, "snr_24h": snr,
            # v2 additions
