"""
AgniPariksha 2.3 — Device & Test Profile Resolver Registry
=========================================================
...
class ProfileRegistry:
    ...
    def _load_profiles(self):
            ...
            with open(device_cfg_path, "r", encoding="utf-8") as f:
                ...
                for key, dev_dict in data.get("device_profiles", {}).items():
                    try:
                        ...
                        logger.warning(
                            "Failed to parse device profile '%s' with v2.3 schema: %s. "
                            "Attempting legacy flat load.", key, str(e)
                        )
                        try:
                            ...
                            logger.error(
                                "Failed to load device profile '%s' with legacy schema: %s. "
                                "Profile will be UNAVAILABLE.", key, str(e2)
                            )
        else:
            logger.warning("Device profiles config not found at %s", device_cfg_path)
            ...
            with open(test_cfg_path, "r", encoding="utf-8") as f:
                ...
                for key, test_dict in data.get("test_profiles", {}).items():
                    try:
                        ...
                        logger.warning(
                            "Failed to parse test profile '%s' with v2.3 schema: %s. "
                            "Attempting legacy flat load.", key, str(e)
                        )
                        try:
                            ...
                            logger.error(
                                "Failed to load test profile '%s' with legacy schema: %s. "
                                "Profile will be UNAVAILABLE.", key, str(e2)
                            )
        else:
            logger.warning("Test profiles config not found at %s", test_cfg_path)

        # Hard fallbacks if config files were missing entirely
        if "DIGITAL_IC" not in self.device_profiles:
            logger.info("DIGITAL_IC profile not found in config — using built-in fallback.")
        ...
        if "THERMAL_BURN_IN" not in self.test_profiles:
            logger.info("THERMAL_BURN_IN test profile not found in config — using built-in fallback.")
        ...
        logger.info(
            "ProfileRegistry loaded: %d device profiles, %d test profiles",
            len(self.device_profiles), len(self.test_profiles)
        )
    ...
    def reload(self):
        ...
        # Re-create via __new__ which will re-load
        ProfileRegistry()
