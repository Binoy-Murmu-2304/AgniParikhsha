"""
AgniPariksha SDK — Offline Command Line Interface (CLI)
=====================================================
Enables local offline execution of context resolution, data validation,
and screening analysis directly from terminal.

Usage:
  python -m agnipariksha_sdk.cli analyze dataset.csv
  python -m agnipariksha_sdk.cli resolve dataset.csv
  python -m agnipariksha_sdk.cli validate dataset.csv
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="AgniPariksha 2.4 Safe ATE Integration SDK CLI (Read-Only)"
    )
    parser.add_argument(
        "action",
    ...
    client = AgniParikshaClient(operator_id=args.operator)
