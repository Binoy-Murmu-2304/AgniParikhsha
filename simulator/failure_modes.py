"""
AgniPariksha 2.1 — Six Failure Mode Injection Engine
===================================================
...
Classes 4 and 5 added per architectural review:
  - Class 4 tests whether AgniPariksha distinguishes physical intermittent degradation from noise.
  - Class 5 tests whether AgniPariksha identifies a broken measurement channel vs. a good component.
"""
...
def inject_intermittent_fault(
    ...
    Tests AgniPariksha's ability to distinguish intermittent physical degradation from random noise.
    """
    timesteps = len(t_ambient)
    t = np.arange(timesteps)
    iddq    = base_iddq * arrhenius_mult + 0.001 * t + rng.normal(0, 0.02, timesteps)
...
def inject_sensor_fault(
    ...
    Tests AgniPariksha's Experiment 4: Can we distinguish component fault from measurement fault?
    """
    timesteps = len(t_ambient)
    t = np.arange(timesteps)
