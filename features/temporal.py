"""
AgniPariksha 2.1 — Temporal & Population Feature Engineering
===========================================================
...
def compute_modified_z_scores(values: pd.Series) -> pd.Series:
    ...
    Default threshold 3.5σ per methodology; AgniPariksha uses it as a flag, not a reject criterion alone.
    """
    med = values.median()
...
def robust_z_score(values: pd.Series) -> pd.Series:
    ...
    Used as primary population anomaly score in AgniPariksha's Module A.
    """
    med = values.median()
    mad = (values - med).abs().median()
    if mad < 1e-8:
...
def detect_frozen_channels(df_telemetry: pd.DataFrame,
    ...
    Experiment 4: Can AgniPariksha distinguish component degradation from instrument fault?
    """
    results = []
