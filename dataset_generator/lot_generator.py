"""
AgniPariksha 2.5 — AgniPariksha Space Qualification Dataset (ASQD) Lot Generator
================================================================================
Generates qualification lots for 5 spaceflight device families using device-specific
physics-based drift models, radiation (TID) degradation, and TVAC cycling:
  - DIGITAL_IC:             ArrheniusIDDQModel      (Ea=0.68 eV [PE]) + TID Ionizing Dose
  - MIXED_SIGNAL_IC:        ArrheniusIDDQModel      (Ea=0.68 eV [PE]) + Dielectric Stress
  - MEMS_GYROSCOPE:         ViscoelasticMEMSModel   (ZRO drift [PE]) + TVAC Creep
  - IMAGE_SENSOR:           SRHDarkCurrentModel     (Dark current [PE]) + TID Radiation Traps
  - PRECISION_VOLTAGE_REF:  ArrheniusIDDQModel      (VOS drift [PE]) + Thermal Hysteresis

Each lot carries the canonical ASQD 2.5 three-dimension schema:
  Component × Test Context × Dense Time-Series Measurement
...
class LotSimulator:
    """
    AgniPariksha Space Qualification Dataset (ASQD) multi-device qualification lot generator.
    ...
    def generate_lot(
        ...
        """
        Generate an ASQD 2.5 lot DataFrame.
        Includes controlled process variation, lot baseline scaling, TID radiation, TVAC cycling, and spatial maps.
        """
        if seed is not None:
            np.random.seed(seed)
        ...
        for comp_idx in range(num_components):
            ...
            # Wafer coordinates & radial edge proximity factor [PE]
            wafer_x = round(float(np.random.uniform(-50, 50)), 2)
            wafer_y = round(float(np.random.uniform(-50, 50)), 2)
            radial_dist = np.sqrt(wafer_x**2 + wafer_y**2)
            # Dies near the wafer periphery experience subtle thermal/CMP process gradient
            edge_factor = 1.0 + (0.05 * (radial_dist / 70.7) if radial_dist > 40 else 0.0)
            ...
            traj = physics_model.generate_trajectory(
                ...
                edge_proximity_factor=edge_factor
            )

            v0   = traj[0]
            v1   = traj[1]   if len(traj) > 1   else v0
            v6   = traj[6]   if len(traj) > 6   else v0
            v12  = traj[12]  if len(traj) > 12  else v0
            v24  = traj[24]  if len(traj) > 24  else traj[-1]
            v48  = traj[48]  if len(traj) > 48  else traj[-1]
            ...
            records.append({
                ...
                # --- ASQD Layer 3: Stress & Mission Environment ---
                ...
                # --- ASQD Layer 4a: Dense Primary Measurements ---
                ...
                "delta_value_0_12h":   round(v12 - v0, 6),
                ...
                **secondary,
                ...
                "wafer_x":             wafer_x,
                "wafer_y":             wafer_y,
                "radial_edge_dist_mm": round(float(radial_dist), 2),
            })
